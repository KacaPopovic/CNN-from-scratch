...
class Pooling(BaseLayer):
    ...
    def backward(self, error_tensor):
        prev_error_tensor = self.max_locations
        error_tensor_flatten = error_tensor.flatten()
        batch, channel, y, x = prev_error_tensor.shape
        counter = 0
        for b in range(batch):
            for c in range(channel):
                for i in range(y):
                    for j in range(x):
                        if prev_error_tensor[b, c, i, j] == 1:
                            prev_error_tensor[b, c, i, j] *= error_tensor_flatten[counter]
                            counter += 1
            ...
            for c in range(channel):
                for i in range(error_tensor.shape[2]):
                    for j in range(error_tensor.shape[3]):
                        ...
                        if len(max_pos[0]) > 0 and len(max_pos[1]) > 0:
                            prev_error_tensor[b, c, start_y + max_pos[0][0], start_x + max_pos[1][0]] = error_tensor[
                                b, c, i, j]
